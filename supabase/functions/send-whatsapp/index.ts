...
serve(async (req) => {
  ...
  try {
    ...
    const { phone, template_key, variables, language = "ar" } = body;

    if (!phone || !template_key) {
      return new Response(
        JSON.stringify({ error: "phone and template_key are required" }),
    ...
    if (templateError || !template) {
      ...
      return new Response(
        JSON.stringify({ error: "Template not found or inactive", template_key }),
    ...
    // Add components if there are variables
    ...
    if (!whatsappResponse.ok) {
      ...
      await supabase.from("sync_logs").insert({
        ...
        sync_details: { phone: formattedPhone, template_key },
      });
